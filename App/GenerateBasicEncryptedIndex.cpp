...
void generate_basic_encrypted_index(const char* plain_posting_file,
  ...
  /*
  ret = ecall_file_open(global_eid, &fp, enc_filename, "w+");
  if (ret != SGX_SUCCESS) {
    std::cout << "Error: ecall_file_open." << std::endl;
    abort();
  }

  uint8_t *input_data = (uint8_t *)malloc(128 * sizeof(uint8_t));
  for (int i = 0; i < 128; i++) input_data[i] = 12;

  size_t write_size = 0;
  ret = ecall_file_write(global_eid, &write_size, fp, input_data, 128);
  if (ret != SGX_SUCCESS) {
    std::cout << "Error: ecall_file_write." << std::endl;
    abort();
  }

  std::cout << "Write " << write_size << " bytes into " << enc_filename << std::endl;
  ...
  ret = ecall_file_open(global_eid, &fp, enc_filename, "r");
  if (ret != SGX_SUCCESS) {
    std::cout << "Error: ecall_file_open." << std::endl;
    abort();
  }
  ...
  ret = ecall_file_get_file_size(global_eid, &file_size, fp);
  if (ret != SGX_SUCCESS) {
    std::cout << "Error: ecall_file_get_size." << std::endl;
    abort();
  }
  
  uint8_t *read_data = (uint8_t *)malloc(file_size * sizeof(uint8_t));
  size_t read_size = 0;
  ret = ecall_file_read(global_eid, &read_size, fp, read_data, file_size);
  if (ret != SGX_SUCCESS) {
    std::cout << "Error: ecall_file_read." << std::endl;
    abort();
  }

  std::cout << "Read data of size " << read_size << " bytes. " << std::endl;

  for (size_t i = 0; i < read_size; i++) std::cout << (int)read_data[i] << " ";
  std::cout << std::endl;

  ret = ecall_generate_encrypted_index(global_eid);
  if (ret != SGX_SUCCESS) {
    std::cout << "Error: ecall_generate_encrypted_index." << std::endl;
    abort();
  }
  
  free(read_data);

  std::cout << "Finish generating encrypted index from " << plain_posting_file << std::endl;
  */
}
